...
gulong
kms_utils_add_buffer_latency_meta_probe (GstPad * pad, gboolean is_valid,
    KmsMediaType type)
{
  BufferLatencyValues *blv;
...
calculate_buffer_latency (GstBuffer * buffer, BufferLatencyData * bl)
{
  ...
  GST_DEBUG_OBJECT (bl->pad, "%s latency %" GST_STIME_FORMAT,
      str_media_type (meta->type), GST_STIME_ARGS (diff));
}
...
gulong
kms_utils_add_buffer_latency_notification_probe (GstPad * pad,
