...
generate_certkey_pem_file (const gchar * cert_key_pem_file)
{
  ...
  cmd =
      g_strconcat
      ("/bin/sh -c \"echo " " > ", CERTTOOL_TEMPLATE,
...
generate_fingerprint_sdp_attr (KmsWebrtcEndPoint * webrtc_end_point)
{
  ...
  if (webrtc_end_point->priv->certificate_pem_file == NULL) {
    ...
    GST_ELEMENT_INFO (webrtc_end_point, RESOURCE, SETTINGS,
        ("\"certificate_pem_file\" property not set, autogenerate a certificate"),
        (NULL));

    uuid = generate_uuid ();
    autogenerated_pem_file = g_strdup_printf ("/tmp/%s.pem", uuid);
    g_free (uuid);

    if (!generate_certkey_pem_file (autogenerated_pem_file)) {
      GST_ELEMENT_ERROR (webrtc_end_point, RESOURCE, SETTINGS,
    ...
    g_object_set (webrtc_end_point, "certificate-pem-file",
        autogenerated_pem_file, NULL);
    webrtc_end_point->priv->autogenerated = TRUE;
    g_free (autogenerated_pem_file);
  }
...
kms_webrtc_end_point_finalize (GObject * object)
{
  ...
  if (self->priv->autogenerated) {
    remove (self->priv->certificate_pem_file);
  }
